...
struct Position
{
   F32 x; ///< X position of the object, from 0 ... 1
   F32 y; ///< Y position of the object, from 0 ... 1
};
...
public:
   ...
   F32 t;                     ///< Parameter of how far the player is along the line from startPos to endPos.
   F32 tDelta;                ///< Change in t per second (ie, velocity).
   ...
   /// changed state.
   void serverSetPosition(Position startPos, Position endPos, F32 t, F32 tDelta);
   ...
   TNL_DECLARE_RPC(rpcPlayerDidMove,   (Float<6> x, Float<6> y));
   ...
   TNL_DECLARE_RPC(rpcPlayerIsInScope, (Float<6> x, Float<6> y));
...
class Building : public NetObject
{
   typedef NetObject Parent;
public:
...
class TestConnection : public GhostConnection
{
...
class TestNetInterface : public NetInterface
{
...
public:
   ...
   U32 lastPingTime;    ///< The last platform time that a GamePingRequest was sent from this network interface.
   bool isServer;       ///< True if this network interface is a server, false if it is a client.

   SafePtr<TestConnection> connectionToServer; ///< A safe pointer to the current connection to the server, if this is a client.
   Address pingAddress; ///< Network address to ping in searching for a server.  This can be a specific host address or it can be a LAN broadcast address.
   ...
   TestNetInterface(TestGame *theGame, bool server, const Address &bindAddress, const Address &pingAddr);
   ...
   void handleInfoPacket(const Address &address, U8 packetType, BitStream *stream);
...
class TestGame {
public:
   Vector<Player *> players;     ///< vector of player objects in the game
   Vector<Building *> buildings; ///< vector of buildings in the game
   bool isServer;                ///< was this game created to be a server?
   TestNetInterface *myNetInterface; ///< network interface for this game
   U32 lastTime; ///< last time that tick() was called
   SafePtr<Player> serverPlayer; ///< the player that the server controls
   SafePtr<Player> clientPlayer; ///< the player that this client controls, if this game is a client
   ...
   TestGame(bool server, const Address &bindAddress, const Address &pingAddress);
