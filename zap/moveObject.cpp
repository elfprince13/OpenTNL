...
void MoveObject::move(F32 moveTime, U32 stateIndex, bool displacing)
{
   ...
   while(moveTime > moveTimeEpsilon && tryCount < 8)
   {
      ...
      else if(objectHit->getObjectTypeMask() & BarrierType)
      {
   ...
   if(isGhost())
   {
      ...
      if(scale > 0.5f)
      {
         ...
         for(S32 i=0; i<4*pow((F32)scale, 0.5f); i++)
         {
            ...
            if(Random::readF() > 0.5)
               SparkManager::emitSpark(collisionPoint, normal * chaos.len() + Point(normal.y, -normal.x)*scale*5  + chaos + mMoveState[stateIndex].vel*0.05f, bumpC);
