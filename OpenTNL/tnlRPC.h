...
public:
	...
	RPCEventImpl () : RPCEvent(gType, dir), mFunctorDecl(remoteF) {
		this->mFunctor = &mFunctorDecl;
	}
	
	static RPCEventImpl* construct(ArgTs ...args){
		auto *theEvent = new SelfType;
		theEvent->mFunctorDecl.set(args ...);
		return theEvent;
	}
	
	static void test(Owner o, ArgTs ...args){
		auto *theEvent = construct(args...);
		TNL::PacketStream ps;
		theEvent->pack(o, &ps);
		ps.setBytePosition(0);
		theEvent->unpack(o, &ps);
		theEvent->process(o);
	}
...
#define TNL_DECLARE_RPC(class, name, groupMask, guaranteeType, eventDirection, rpcVersion, ...) \
virtual void name##_remote (__VA_ARGS__);\
static constexpr const char RPC_##class##_##name##Str[] = "RPC_" #class "_" #name; \
TNL::RPCDispatch<class, RPC_##class##_##name##Str, decltype(& class :: name##_remote), & class :: name##_remote, groupMask, rpcVersion, guaranteeType, eventDirection, ##__VA_ARGS__> name;


#define TNL_IMPLEMENT_RPC(className, name, args) \
...
#define TNL_IMPLEMENT_RPC_OVERRIDE(className, name, args) TNL_IMPLEMENT_RPC(className, name, args)
...
#define TNL_RPC_CONSTRUCT_NETEVENT(object, rpcMethod, args) (object)->rpcMethod::construct args
